...
<nav class="border-b bg-background sticky top-0 z-50">
	<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
		<div class="flex h-16 items-center justify-between">
			<div class="flex items-center">
				<a href="/" class="shrink-0" aria-label="home">
					<svg
						...
						><rect width="18" height="18" x="3" y="3" rx="2" /><path d="M8 16V8l4 4 4-4v8" /></svg
					>
				</a>
				<div class="hidden sm:ml-6 sm:block">
					<div class="flex space-x-4">
						{#each navs as nav}
							<Button variant="ghost" href={nav.url}>{nav.name}</Button>
						{/each}
					</div>
				</div>
			</div>
			...
			<div class="-mr-2 flex sm:hidden">
				...
				<button
					type="button"
					class="relative inline-flex items-center justify-center rounded-md p-2 text-muted-foreground"
	...
	{#key isMobileMenu}
		<div class="sm:hidden {isMobileMenu ? 'block' : 'hidden'} " id="mobile-menu" transition:slide>
			<div class="px-2 pb-3 pt-2">
				<div>
					{#each navs as nav}
						<a
							href={nav.url}
							class="block rounded-md px-3 py-2 text-base font-normal text-muted-foreground"
							>{nav.name}</a
						>
					{/each}
				</div>
